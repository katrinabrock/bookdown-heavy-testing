...
invisible(lapply(c('main', 'test'), function(scenario) {
  env_dir <- paste0(scenario, '-env')
  if(dir.exists(env_dir)) unlink(env_dir, recursive = TRUE)
  R.utils::copyDirectory('base-env', env_dir, recursive = TRUE)
  renv::load(env_dir)
  devtools::install(paste0('bookdown-', scenario))
  sapply(names(books_to_test), function(book) {
    output_dir <- file.path(getwd(), paste0(scenario, '-out'), book)
    dir.create(output_dir, recursive = TRUE)
    bookdown::render_book(
      input = file.path(book, books_to_test[[book]]),
